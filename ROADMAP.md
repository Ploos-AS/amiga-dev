...
M7.4 verifies the exact published digest after publication: the GitHub build-provenance attestation is verified against the repository, and BuildKit SBOM and provenance metadata are required to be present and valid JSON for that same OCI digest.
...
M8.1 qualification evidence: GitHub Actions CI run #86 (`35266683926`) and Publish OCI image run #45 (`35266683693`) completed successfully on 2026-09-17 for commit `f65007f9ab8f6a12816ac4a83fc4efcc6098384a`. The machine-readable `consumer-contract.json` and its CI validation passed together with the existing qualified image regression suite and publication path.

M8.2 qualification evidence: GitHub Actions CI run #90 (`35267825353`) and Publish OCI image run #49 (`35267825383`) completed successfully on 2026-09-17 for commit `efd7a397838b20338279a55bb422b186e1d93d40`. CI validated the GitHub and Forgejo consumer migration templates, default 68000 profile, stable `amiga-*` command interface, image-reference guidance and the documented separation from `amiga-runtime`.
